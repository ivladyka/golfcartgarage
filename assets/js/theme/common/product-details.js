...
export default class ProductDetails {
    ...
    productOptionsChanged(event) {
        ...
        utils.api.productAttributes.optionChange(productId, $form.serialize(), 'products/bulk-discount-rates', (err, response) => {
            ...
            // ===== Extend SDK ===== //
            Extend.buttons.instance('#extend-offer').setActiveProduct(productAttributesData.sku)
            // ===== End Extend SDK ===== //
        });
    }
    ...
    updatePriceView(viewModel, price) {
		...
		/* 20190911 ysw - add update for stickyATC */
		var stickyATCprice = $('#stickyATC .price--withoutTax');
        if (price.with_tax) {
        ...
        if (price.without_tax) {
			...
			if (stickyATCprice){
				stickyATCprice.html(price.without_tax.formatted);
			} 
